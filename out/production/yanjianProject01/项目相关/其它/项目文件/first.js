...
function setInnerHTML(inID, inInnerHTML) {
    ...
    if ((inInnerHTML.indexOf("POPUP_") == 0 || inInnerHTML.indexOf("LOCK_") == 0 || inInnerHTML.indexOf("#") == 0 || inInnerHTML.indexOf("%") == 0) && (first_LastMessage != inInnerHTML || (nowTime - first_EchoMessageTime) > first_OutTime)) {
        setTimeout(function () {
            ...
            } else if (inInnerHTML.indexOf("LOCK_") == 0 || inInnerHTML.indexOf("%") == 0) {
    ...
    } else if (inInnerHTML.indexOf("LOCK_") == 0) {
        obj.innerHTML = inInnerHTML.replace("LOCK_", "");
    } else if (inInnerHTML.indexOf("MESSAGE_") == 0) {
        obj.innerHTML = inInnerHTML.replace("MESSAGE_", "");
    } else if (inInnerHTML.indexOf("#") == 0) {
...
function first_AndroidKeyPressAction(value) {
    ...
    if (value != null && value.length > 0) {
        ...
        } else if (obj != null && value == "DEL" && obj.value.length > 0) {
            if (getCurContext == keyValue) {
                setValue(id, "");
            } else {
...
function first_MessageSystem(messageString, musicType) {
    ...
    if (messageString.indexOf(first_MessageSeparator) > 0) {
        ...
        if (messageString.indexOf("#") == 0) {
            ...
            type = "POPUP";
            music = "-1";
        } else if (messageString.indexOf("%") == 0) {
            message = messageString.replace("%", "");
            type = "LOCK";
            music = "-1";
        } else {
...
function first_BodyOnKeyUpAction(event) {
    ...
    } else if (event.keyCode == first_CancelKeyNumber) {
        first_DoPageCancelAction(event);
    } else if (event.keyCode == first_GetLastKeyNumber) {
        first_DoPageGetLastAction(event);
    } else if (event.keyCode == first_GetNextKeyNumber) {
        first_DoPageGetNextAction(event);
    } else if (event.keyCode == first_Udf1KeyNumber) {
        first_DoPageUdf1Action(event);
    } else if (event.keyCode == first_Udf2KeyNumber) {
        first_DoPageUdf2Action(event);
    } else if (event.keyCode == first_Udf3KeyNumber) {
...
function setDisplay(inID, inDisplay) {
    if (document.getElementById(inID) != null) document.getElementById(inID).style.display = inDisplay;
};
...
function setClassName(inID, inClassName) {
    if (document.getElementById(inID) != null) document.getElementById(inID).className = inClassName;
};
...
function hideAll(inViewsKeys) {
    ...
    ;
    if (inViewsKeys == null || inViewsKeys.length <= 0) return;
...
function addOptionToSelect(objId, optValue, optText, isNotSelected) {
    ...
    if (isNotSelected != null && isNotSelected) {
        ...
        if (optValue == null || optValue == "") return;
        if (optText == null || optText == "") optText = optValue;
    }
...
function isNumber(str) {
    ...
    str += "";
    if ((str.charAt(0) == "0" && str.length > 1 && str.charAt(1) != ".") || str.charAt(0) == "." || str.charAt(str.length - 1) == "." || str.indexOf(".") != str.lastIndexOf(".") || parseFloat(str) < 0) return false;
...
function first_EchoMessage(message, type, functionName) {
    ...
    } else if (message.indexOf("LOCK_") == 0) {
        message = message.replace("LOCK_", "");
    } else if (message.indexOf("MESSAGE_") == 0) {
