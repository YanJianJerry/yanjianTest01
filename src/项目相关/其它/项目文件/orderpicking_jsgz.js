...
var hide_serialNoBuf_3 = "hide_serialNoBuf_3";
...
var Pck_Lot_Cfm_Use = "";
var hide_warehouseId = "hide_warehouseId";
...
var buf_SkuArray = new Array();
var buf_TaskIdArray = new Array();
var buf_UomAndQty_4 = new Array();
var buf_InvQty_4 = 0;
var buf_hasScanFlag_4 = false;
...
var wxt_Comments_1 = "wxt_Comments_1";
var wxt_Notes_1 = "wxt_Notes_1";
...
var but_QuerySku_3 = "but_QuerySku_3";
...
var buf_skuMessageObj_7 = null;
var skuList_7 = new Array();
var chooseIndex = -1;
...
function initHtmlPage() {
    ...
    setValue(hide_Pck_Lst_Seq, first_GetParameterValue(PCK_LST_SEQ, "*", "*", p_warehouseId));
    autoFocus("init");
}
...
function initPageOne() {
    ...
    var wxt_Notes_1_node = document.getElementById(wxt_Notes_1);
    var father_node = wxt_Notes_1_node.parentElement;
    var sfcValue_notes = sfc_GetSfcValueById_Private(wxt_Notes_1, sfc_Code_1);
    var isDisPlay_notes = sfc_IsDisPlay_Private(sfcValue_notes);
    var wxt_Comments_1_node = document.createElement("textarea");
    wxt_Comments_1_node.setAttribute("id", wxt_Comments_1);
    wxt_Comments_1_node.setAttribute("class", "textarea1");
    wxt_Comments_1_node.setAttribute("readonly", "true");
    wxt_Comments_1_node.setAttribute("rows", "15");
    father_node.replaceChild(wxt_Comments_1_node, wxt_Notes_1_node);
    setValue(wxt_Comments_1, "");
    if (!isDisPlay_notes) {
        wxt_Comments_1_node.style.display = "none";
    }
}
...
function first_ActionControl(id, event) {
    if (event == null || event.keyCode == 13) {
        ...
        if (id == but_QuerySku_3) {
            but_QuerySku_3_Action(id);
            return;
        }
        ...
        if (id == ScanByEach_Txt_Sku) {
            ScanByEach_Txt_Sku_Action(event);
            return;
        }
        ...
        alert("OrderPicking function first_ActionControl() is Error: " + id + " not found.");
    }
}
    ...
    if (curSEQ > first_lastSEQUpdate1) {
        ...
        if (codeValue_OK == replyData.codeValue) {
            ...
            for (var i = 0; i < obj2.length; i++) {
                buf_OrderList_2.push(new Array(obj2[i].ORDERNO));
            }
            Table_CreateTable(div_OrderNoList_2, new Array(getValue(hide_Lbl_01)), buf_OrderList_2, 7, "doOrderListSingleClickAction", "doOrderListSingleDoubleAction", "doOrderListSingleDeleteAction", false, false);
...
function txt_OrderNo_1_Action(id) {
    ...
    _prevView = _div1Input;
    showViewSingle(_div1Waiting, inViewsKeys);
    doServerAction(className, "getOrderTypeAction", QSKeys, getOrderTypeAction);
}

function getOrderTypeAction(responseText) {
    var replyData = null;
    try {
        replyData = eval("(" + responseText + ")");
    } catch (e) {
    ...
    if (curSEQ > first_lastSEQUpdate1) {
        ...
        if (codeValue_OK == replyData.codeValue) {
            ...
            setValue(hide_Pck_Lst_Seq, first_GetParameterValue(getValue2(hide_Pck_Lst_Seq), obj1.CUSTOMERID, obj1.ORDERTYPE, p_warehouseId));
            doServerAction(className, "getCustomerNotesAction", QSKeys, getCustomerNotesAction);
        } else {
            if (codeValue_NORECORD == replyData.codeValue) {
                showViewSingle(_prevView, inViewsKeys);
            } else {
...
function getCustomerNotesAction(responseText) {
    var replyData = null;
    try {
        replyData = eval("(" + responseText + ")");
    } catch (e) {
    ...
    if (curSEQ > first_lastSEQUpdate1) {
        ...
        if (codeValue_OK == replyData.codeValue) {
            var obj1 = replyData.strData1;
            setValue(wxt_Comments_1, obj1.NOTES);
            showViewSingle(_prevView, inViewsKeys);
            autoFocus(sfc_GetNextId(txt_OrderNo_1, sfc_Code_1));
        } else {
            if (codeValue_NORECORD == replyData.codeValue) {
                showViewSingle(_prevView, inViewsKeys);
            } else {
...
function getOrderInfoAction(responseText) {
    ...
    if (curSEQ > first_lastSEQUpdate1) {
        ...
        if (codeValue_OK == replyData.codeValue) {
            var obj1 = replyData.strData1;
            var obj2 = replyData.strData2;
            setValue(txt_OrderNo_1, returnMessage);
            setValue(hide_OrderNo_1, returnMessage);
...
function but_Confirm_2_Action(id) {
    ...
    if (index >= 0) {
        setValue(txt_OrderNo_1, buf_OrderList_2[index]);
    }
    but_Cancel_2_Action(id);
}
...
function txt_FmLocation_3_Action(id) {
    ...
    if (fmLocationValue == getValue2(wxt_FmLocation_3)) {
        if (getValue(wxt_Sku_3) == getValue(txt_Sku_3)) {
            autoFocus(txt_Sku_3);
            return;
        }
        startMusic2();
    ...
    if (fmTraceIdValue.length == 0 || fmTraceIdValue == getValue2(wxt_FmTraceId_3)) {
        ...
        if (getValue2(hide_Pck_Chg_Id) != "Y") {
            ...
            if (startMusic() || !first_EchoMessage(getValue(hide_Message_31), first_Confirm, "")) {
                ...
                setValue(hide_NewFmTraceId_3, fmTraceIdValue);
                _prevView = _div3Input;
                showViewSingle(_div1Waiting, inViewsKeys);
...
function txt_Sku_3_Action(id) {
    ...
    SkuBarcodeAnalyzer_SetParameterAttribute(customerId, "*", p_warehouseId);
...
function returnFunction() {
    ...
    if (returnCode == codeValue_OK) {
        ...
        if (skuValue == getValue2(wxt_Sku_3)) {
            ...
            if (buf_OrderInfo_3 != null && buf_OrderInfo_3.length > 0) {
                for (var i = 0; i < buf_OrderInfo_3.length; i++) {
                    if (buf_OrderInfo_3[i].SKU == skuOutput && buf_OrderInfo_3[i].TASKID == taskId) {
                        buf_OrderIndex_3 = i;
                        showPageThreeAction();
                        break;
                    }
...
function PublicJs_dynamicLot_ConfirmLotButton() {
    if (PublicJs_dynamicLot_CheckLotattValue() == null) {
        var order = buf_OrderInfo_3[buf_OrderIndex_3];
        var isScanSerialNo1 = order.SCANWHENCASEPICKING;
        var isScanSerialNo2 = order.SCANWHENCASEPICKING;
        if (isScanSerialNo1 == "Y" || isScanSerialNo2 == "Y") {
            showPage4Action();
...
function ScanningSerialNo_But_LoadSerialNo_Action() {
    var order = buf_OrderInfo_3[buf_OrderIndex_3];
    var control = order.OUTBOUNDSERIALNOQTYCONTROL;
    buf_hasScanFlag_4 = true;
    var serialNoArrays = ScanningSerialNo_GetScannedSerialNo();
    var scanQty = serialNoArrays.length;
    var inputQty = parseFloat(order.FMQTY);
    if (control == "MATCH" && !ScanningSerialNo_IsGiveUp()) {
        if (scanQty != inputQty && !first_EchoMessage(getValue(hide_Message_24), first_Alert, "updateMessageBar")) {
    ...
    if (serialNoArrays.length > 0) {
        var serialNoValues = "";
        for (var i = 0; i < serialNoArrays.length; i++) {
            serialNoValues += serialNoArrays[i][0] + ";";
        }
        setValue(hide_SerialNoList_4, serialNoValues);
    } else {
        ScanningSerialNo_ClearSerialNoInputText();
        ScanningSerialNo_FocusSerialNoInputText();
        setValue(hide_SerialNoList_4, "");
    }
    ...
    if (curSEQ > first_lastSEQUpdate1) {
        ...
        if (codeValue_OK == replyData.codeValue) {
            ...
            if (codeValue_NORECORD == replyData.codeValue) {
                ...
                playMusic("ERROR");
                if (buf_hasScanFlag_4 && getValue(hide_SerialNoList_4).length > 0) {
...
function but_QuerySku_3_Action(id) {
    _prevView = _div3Input;
    showViewSingle(_div1Waiting, inViewsKeys);
    var QSKeys = new Array(txt_OrderNo_1);
    doServerAction(className, "getSkuInfoAction", QSKeys, getSkuInfoAction);
}

function getSkuInfoAction(responseText) {
    var replyData = null;
    try {
        replyData = eval("(" + responseText + ")");
    } catch (e) {
...
function but_StockOut_3_Action(id) {
    if (!first_EchoMessage(getValue2(hide_Message_41), first_Confirm, "")) {
        setFocus2(but_StockOut_3);
        return;
    }
    var order = buf_OrderInfo_3[buf_OrderIndex_3];
...
function showPage4Action() {
    ...
    sfc_SetValue(txt_Qty_4, order.FMQTY, sfc_Code_4);
    sfc_SetValue(wxt_Uom_4, order.DESCR + "(" + order.FMUOM + ")", sfc_Code_4);
    sfc_SetValue(cmb_Uom_4, order.FMUOM, sfc_Code_4);
    sfc_SetValue(cmb_ReasonCode_4, "OK", sfc_Code_4);
    sfc_SetValue(wxt_PackId_4, order.FMPACKID, sfc_Code_4);
...
function but_SingleScan_4_Action(id) {
    ...
    var taskId = order.TASKID;
    var customerId = order.CUSTOMERID;
    var skuInfo = new Array(getValue2(wxt_Sku_3), getValue2(wxt_SkuName_3), 1, getValue2(wxt_Qty_4), "PCK", taskId, "", customerId, "");
...
function but_Confirm_4_Action(id) {
    ...
    if (nextId == sfc_Code_4) {
        ...
        if (isScanSerialNo1 == "Y" && isScanSerialNo2 == "Y" && isScanSerialNo3 == "Y" && !buf_hasScanFlag_4) {
            order.FMQTY = qtyValue;
            ScanningSerialNo_SetSkuInfo(order.DOCTYPE, order.SKU, order.CUSTOMERID);
            ScanningSerialNo_Init(inViewsKeys, parseFloat(order.FMQTY), order.FMUOM, _div3Input, true, new Array(), "OUT");
        } else {
            _prevView = _div4Input;
            showViewSingle(_div1Waiting, inViewsKeys);
            doServerAction(className, "doPickingAction", QSKeys, doPickingAction);
...
function showPage5Action(obj1, obj2, isRefresh) {
    ...
    sfc_SetValue(cmb_Uom_5, order.FMUOM, sfc_Code_5);
...
function txt_FmTraceId_5_Action(id) {
    ...
    if (fmTraceIdValue.length == 0 || fmTraceIdValue == getValue2(wxt_FmTraceId_5)) {
        ...
        if (getValue2(hide_Pck_Chg_Id) != "Y" && false) {
            ...
            if (startMusic() || !first_EchoMessage(getValue(hide_Message_31), first_Confirm, "")) {
                ...
                setValue(hide_NewFmTraceId_5, fmTraceIdValue);
                _prevView = _div5Input;
                showViewSingle(_div1Waiting, inViewsKeys);
...
function show_SingleScan_5_Action() {
    ...
    var taskId = order.TASKID;
    var customerId = order.CUSTOMERID;
    var wxtQty5 = order.FMQTY;
    if (getValue(hide_Qty_5).length != 0) {
        wxtQty5 = getValue(hide_Qty_5);
        setValue(txt_Qty_5, getValue(hide_Qty_5));
        setValue(hide_Qty_5, "");
    }
    if (wxtQty5 > 1) {
        var skuInfo5 = new Array(getValue2(wxt_Sku_5), getValue2(wxt_SkuName_5), 1, wxtQty5, "PCK", taskId, "", customerId, "");
        ...
        ScanByEach_ShowScanByEachPage(inViewsKeys, skuInfo5, buttonActions5, otherInfo5);
        return;
    } else {
        showViewSingle(_div5Input, inViewsKeys);
        setValue(txt_Qty_5, wxtQty5);
...
function showSkuInfo() {
    skuList_7 = new Array();
    for (var i = 0; i < buf_skuMessageObj_7.length; i++) {
        skuList_7.push(new Array(buf_skuMessageObj_7[i].SKU, buf_skuMessageObj_7[i].DESCR));
    }
    Table_CreateTable(div_SkuList_7, new Array(getValue3("hide_Lbl_02"), getValue3("hide_Lbl_03")), skuList_7, 8, "getSkuSingleAction", "getSkuDoubleAction", "", false, false);
}

function getSkuSingleAction(index) {
    updateMessageBar("");
    chooseIndex = index;
    setFocus(but_Confirm_7);
}

function getSkuDoubleAction(index) {
    chooseIndex = index;
...
function but_Confirm_7_Action(id) {
    ...
    if (flag) {
        ...
        setFocus2(div_SkuList_7);
        return;
    }
    showViewSingle(_div3Input, inViewsKeys);
    setValue(txt_Sku_3, skuList_7[chooseIndex][0]);
    txt_Sku_3_Action("");
}
