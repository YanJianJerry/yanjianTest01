...
function getOrderNoListAction(responseText) {
    ...
    if (curSEQ > first_lastSEQUpdate1) {
        ...
        if (codeValue_OK == replyData.codeValue) {
            ...
            Table_CreateTable(div_OrderNoList_2, buf_LabelList, buf_OrderList_2, 7.8, "doOrderListSingleClickAction", "doOrderListSingleDoubleAction", "doOrderListSingleDeleteAction", false, false);
...
function txt_Sku_3_Action(id) {
    ...
    SkuBarcodeAnalyzer_SetAddWhere(" AND ALLOWSHIPMENT='Y' AND BAS_SKU.SKU IN (SELECT SKU FROM DOC_ORDER_DETAILS WHERE ORDERNO='" + getValue2(txt_OrderNo_1) + "')", getValue2(txt_OrderNo_1), "PCK");
...
function do_SerialNoControl_But_Back_1_Action() {
    ifScanQickHasDone = "2";
        ...
        if (codeValue_OK == replyData.codeValue) {
            ...
            if (codeValue_NORECORD == replyData.codeValue) {
                ...
                if (returnMessage == "CHECKNOLOCATION") {
                    ...
                    if (ifScanQickHasDone != "3" && SerialNoControl_GetScanSerialNoQty() > 0) {
...
function checkQtyAndUomAciton(id) {
    ...
    if (parseFloat(qtyValue) * input_UomQty != parseFloat(qty) * db_UomQty) {
        if (id == txt_Qty_4 && sfc_CanFocus(cmb_Uom_4, sfc_Code_4) && sfc_IsCursorstay_Private(sfc_GetSfcValueById_Private(cmb_Uom_4, sfc_Code_4))) {
            autoFocus(cmb_Uom_4);
            return;
        }
        if (sfc_CanFocus(cmb_ReasonCode_4, sfc_Code_4) && sfc_IsCursorstay_Private(sfc_GetSfcValueById_Private(cmb_ReasonCode_4, sfc_Code_4))) {
...
function but_Confirm_4_Action(id) {
    ...
    if (nextId == sfc_Code_4) {
        ...
        if (ifScanQickHasDone != "3" && isScanSerialNo1 == "Y" && isScanSerialNo2 == "Y" && (isScanSerialNo3 == "Y" || isScanSerialNo3 == "M")) {
            var type = SerialNoControl_C_OUT;
            var scanInfo = new Array(order.SKU, order.CUSTOMERID, getValue(txt_Qty_4), getValue(cmb_Uom_4), "*", order.LOTATT12, isScanSerialNo3);
            var noList = new Array(getValue3(hide_TaskId), getValue3(hide_TaskId_Sequence));
            SerialNoControl_Init(serialNoConfirmAction, serialNoCancelAction, type, scanInfo, noList, ifScanQickHasDone);
            return;
        }
        doCheck1Action();
    } else {
